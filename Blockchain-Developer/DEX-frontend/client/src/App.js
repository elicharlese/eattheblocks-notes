...
function App({web3, accounts, contracts}) {
  ...
  const listenToTrades = token => {
    ...
    setTrades([]); // clean out the array of trades
  ...
  const deposit = async amount => {
    ...
    await contracts.dex.methods
      .deposit(
        amount,
        web3.utils.fromAscii(user.selectionToken.ticker)
    )
      .send({from: user.accounts[0]});
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setTokens(tokens);
      setUser({accounts, balances, selectedTokens: tokens[0]});
      setOrders(orders);
    }
    init();
  }, []);
  ...
  useEffect(() => {
    const init = async() => {
      ...
      listenerToTrades(user.selectedToken);
      setUser(user => ({...user, balances}));
      setOrders(tokens[0])
    }
