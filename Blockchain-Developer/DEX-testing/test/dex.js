...
contract('Dex', () => {
  ...
  beforeEach(async() => {
    ...
    const seedToTokenbalance = async (token, trader) => {
    ...
    await Promise.all(
      [dai, bat, rep, zrx].map(
        token => seedToBalance(token, trader1)
      )
    );
    
    await Promise.all(
      [dai, bat, rep, zrx].map(
        token => seedToBalance(token, trader2)
      )
    );
  });
  ...
  it('should depoist tokens', async () => {
  ...
  it('should NOT depost tokens if token does not exist', async () => {
    await expectRevert(
      dex.deposit(
  ...
  /* ============ TESTING ORDER BOOK PURGE FUNCTION ============ */
